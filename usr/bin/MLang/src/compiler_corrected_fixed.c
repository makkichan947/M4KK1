/**
 * M4KK1 MLang Compiler - Corrected Implementation (Fixed)
 * MLang独特编译器修正实现（修正版）
 */
...
int mlang_lexical_analyze(const char *source, mlang_ast_node_t **tokens) {
    ...
    while (pos < len) {
        ...
        if (isalpha(c) || c == '_') {
            ...
            mlang_ast_node_t **token_ptr = mlang_malloc(sizeof(mlang_ast_node_t*));
            *token_ptr = token;
            mlang_array_append(token_array, token_ptr);
            continue;
        }
    ...
    *tokens = mlang_malloc(mlang_array_size(token_array) * sizeof(mlang_ast_node_t*));
    ...
    for (uint32_t i = 0; i < mlang_array_size(token_array); i++) {
        mlang_ast_node_t *token_ptr = (mlang_ast_node_t*)mlang_array_get(token_array, i);
        (*tokens)[i] = *token_ptr;
    }
...
int mlang_compile(mlang_config_t *config, mlang_result_t *result) {
    ...
    /* 生成中间表示 */
    mlang_ir_function_t *ir_functions_ptr;
    int ir_count = mlang_generate_ir(ast, &ir_functions_ptr);
    if (ir_count < 0) {
    ...
    if (mlang_optimize_ir(ir_functions_ptr, config->opt_level) != 0) {
        result->error_message = mlang_strdup("IR optimization failed");
        mlang_free_ir(ir_functions_ptr);
        mlang_free_ast(ast);
        mlang_free(source);
        return -1;
    }
    ...
    uint8_t *code;
    uint32_t code_size;
    if (mlang_generate_code(ir_functions_ptr, config->target_arch, &code, &code_size) != 0) {
