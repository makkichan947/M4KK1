/**
 * M4KK1 MLang Compiler - Utility Functions (Fixed)
 * MLang编译器工具函数实现（修正版）
 */
...
/* 全局统计信息 */
static mlang_stats_t compiler_stats = {0};
...
void mlang_free(void *ptr) {
    ...
    if (ptr) {
        /* 检查是否在内存池中 */
        if ((char *)ptr >= (char *)mlang_memory_pool &&
            (char *)ptr < (char *)mlang_memory_pool + 1024 * 1024) {
            /* 在内存池中，不释放 */
            return;
        } else {
            /* 使用系统malloc分配的，释放 */
...
void mlang_free_ast(mlang_ast_node_t *ast) {
    ...
    if (ast->children) {
        for (uint32_t i = 0; i < ast->child_count; i++) {
            mlang_free_ast(ast->children[i]);
        }
        mlang_free(ast->children);
    }
...
void mlang_free_ir(mlang_ir_function_t *ir_functions) {
    if (!ir_functions) return;
    
    // 这里应该释放IR相关的资源
    // 由于结构体定义中的ir_code是void*，我们假设它需要释放
    if (ir_functions->ir_code) {
        mlang_free(ir_functions->ir_code);
    }
    
    if (ir_functions->function_name) {
        mlang_free(ir_functions->function_name);
    }
    
    mlang_free(ir_functions);
}
